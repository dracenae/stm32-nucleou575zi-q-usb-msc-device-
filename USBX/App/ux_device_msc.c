...
#define STORAGE_BLK_NBR  64  // 32 KB drive size
...
__attribute__ ((section (".usb_data"))) volatile uint8_t ram_disk_buffer[STORAGE_BLK_NBR*STORAGE_BLK_SIZ];
/* USER CODE END PRIVATE_VARIABLES */
//volatile uint8_t ram_disk_buffer[STORAGE_BLK_NBR * STORAGE_BLK_SIZ];

/*uint8_t buffer[512 * 200];  // 100 KB RAM buffer*/
/* USER CODE END PV */
                       ...
                       ULONG number_blocks, ULONG lba, ULONG *media_status)
{
 ...
 // UX_PARAMETER_NOT_USED(lba);
  ...
  uint32_t address = lba * STORAGE_BLK_SIZ;
  /*  if ((address + (number_blocks * STORAGE_BLK_SIZ)) > (STORAGE_BLK_NBR * STORAGE_BLK_SIZ))
    {
        status = UX_ERROR;
    }
    else*/
    {
        memcpy(data_pointer, (void *)&ram_disk_buffer[address], number_blocks * STORAGE_BLK_SIZ);
 ...
 // UX_PARAMETER_NOT_USED(lba);
  ...
  uint32_t address = lba * STORAGE_BLK_SIZ;
   /* if ((address + (number_blocks * STORAGE_BLK_SIZ)) > (STORAGE_BLK_NBR * STORAGE_BLK_SIZ))
    ...
    {
        memcpy((void *)&ram_disk_buffer[address], data_pointer, number_blocks * STORAGE_BLK_SIZ);
        status = UX_STATE_NEXT;
    }
